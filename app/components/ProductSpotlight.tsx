...
const ProductSpotlight = ({
  ...
  // active one. loop on so the ends are not lopsided, the rail wraps round and the
  // centred pack always has neighbours on both sides.
  ...
  useEffect(() => {
    ...
    const tween = () => {
      ...
      snaps.forEach((snap, i) => {
        ...
        if (!node) return;
        const distance = Math.abs((snap - progress) / step);
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="flex gap-4  h-dvh w-screen max-w-none flex-col items-center justify-between overflow-hidden bg-primary/40 backdrop-blur-sm  py-6 pt-12  duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] data-[state=closed]:slide-out-to-right-24 data-[state=open]:slide-in-from-right-24 
      ">
        ...
        <div className="flex min-h-0 w-full flex-1 justify-center px-6 ">
          <div
            className=" flex h-full w-full max-w-md sm:max-w-sm flex-col rounded-3xl
            ...
            sm:border-0 sm:bg-transparent sm:backdrop-blur-none sm:shadow-none
        "
          >
            {/* white screen, the growing part. min-h-0 lets it shrink, the pack
                image contains inside so it never crops */}
            <div className="relative min-h-0 flex-1 overflow-hidden rounded-3xl bg-white px-4 pt-4 pb-8 shadow-2xl ">
              <Image
                src={current.image}
            ...
            <div className="relative shrink-0">
              ...
              <DialogTitle className="font-display text-3xl capitalize leading-none sm:text-4xl px-4 py-6 text-center">
                {current.text}
              </DialogTitle>
            </div>
          </div>
          ...
          <div className="flex items-center">
            {items.map((item, i) => {
              ...
              return (
                <div
                  key={item.text + i}
                  className="flex shrink-0 grow-0 basis-1/4 justify-center px-2 sm:basis-1/5 py-1"
                >
                  <button
                    type="button"
